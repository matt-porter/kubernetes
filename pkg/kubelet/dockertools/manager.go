...
func (dm *DockerManager) runContainer(
	...
	dm.probeManager.HandleNewContainer(kubecontainer.Container{
		ID:      types.UID(dockerContainer.ID),
		Name:    container.Name,
		Image:   dockerContainer.Image,
		Created: dockerContainer.Created.Unix(),
		PodUID:  pod.UID,
	})
...
func (dm *DockerManager) computePodContainerChanges(pod *api.Pod, runningPod kubecontainer.Pod, podStatus api.PodStatus) (PodContainerChangesSpec, error) {
	...
	for index, container := range pod.Spec.Containers {
		...
		if dm.probeManager.GetLiveness(c.ID) == prober.Success {
